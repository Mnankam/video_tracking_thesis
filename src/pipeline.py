...
class VideoPipeline:
    ...
    def _save_binary_masks(
        ...
        particle_bed_raw_mask=None,
        inner_pipe_mask=None,
        inner_pipe_raw_mask=None,
        combined_mask=None,
    ) -> None:
        """
        Save binary masks generated by the pipeline.

        The standard filenames are kept unchanged for compatibility with the
        existing IoU evaluation scripts:

        - frame_XXXXXX_particle_bed.png
        - frame_XXXXXX_inner_pipe.png
        - frame_XXXXXX_combined.png

        If postprocessing is enabled, additional masks are stored to support
        before/after visualization in the thesis:

        - frame_XXXXXX_particle_bed_raw.png
        - frame_XXXXXX_particle_bed_postprocessed.png
        - frame_XXXXXX_inner_pipe_raw.png
        - frame_XXXXXX_inner_pipe_postprocessed.png
        """
        if not self.config.save_binary_masks:
            return

        if particle_bed_raw_mask is not None and self.config.enable_postprocessing:
            raw_binary = self._to_binary_mask(particle_bed_raw_mask)
            raw_path = os.path.join(
                self.config.binary_mask_dir,
                "particle_bed",
                f"frame_{frame_idx:06d}_particle_bed_raw.png",
            )
            cv2.imwrite(raw_path, raw_binary)

        if particle_bed_mask is not None:
            binary = self._to_binary_mask(particle_bed_mask)

            # Keep original output name for existing evaluation scripts.
            ...
            if self.config.enable_postprocessing:
                pp_path = os.path.join(
                    self.config.binary_mask_dir,
                    "particle_bed",
                    f"frame_{frame_idx:06d}_particle_bed_postprocessed.png",
                )
                cv2.imwrite(pp_path, binary)

        if inner_pipe_raw_mask is not None and self.config.enable_postprocessing:
            raw_binary = self._to_binary_mask(inner_pipe_raw_mask)
            raw_path = os.path.join(
                self.config.binary_mask_dir,
                "inner_pipe",
                f"frame_{frame_idx:06d}_inner_pipe_raw.png",
            )
            cv2.imwrite(raw_path, raw_binary)
        ...
        if inner_pipe_mask is not None:
            ...
            if self.config.enable_postprocessing:
                pp_path = os.path.join(
                    self.config.binary_mask_dir,
                    "inner_pipe",
                    f"frame_{frame_idx:06d}_inner_pipe_postprocessed.png",
                )
                cv2.imwrite(pp_path, binary)
        ...
        if combined_mask is not None:
            ...
            if self.config.enable_postprocessing:
                pp_path = os.path.join(
                    self.config.binary_mask_dir,
                    "combined",
                    f"frame_{frame_idx:06d}_combined_postprocessed.png",
                )
                cv2.imwrite(pp_path, binary)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        with open(self.config.output_csv, "w", newline="", encoding="utf-8") as f, \
            ...
            while frame_idx < end:
                ...
                raw_particle_bed_mask = particle_bed_mask.copy()
                ...
                if self.inner_pipe_segmenter is not None:
                    ...
                    raw_inner_pipe_mask = inner_pipe_mask.copy()
                ...
                if self.config.save_binary_masks:
                    self._save_binary_masks(
                        ...
                        particle_bed_raw_mask=raw_particle_bed_mask,
                        inner_pipe_mask=inner_pipe_mask,
                        inner_pipe_raw_mask=raw_inner_pipe_mask,
