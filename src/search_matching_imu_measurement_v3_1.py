...
def main() -> None:
    ...
    exploratory_matches = time_window_analysis[
        time_window_analysis["number_simultaneous_candidates"] > 0
    ].copy()

    if len(simultaneous):
        conclusion = (
            f"{len(simultaneous)} measurement(s) satisfy both temporal and "
            f"spectral criteria within the configured ±{args.time_margin_s / 60.0:g} "
            "minute time margin."
        )
        earliest_exploratory_window_minutes = args.time_margin_s / 60.0
        earliest_exploratory_candidate_count = int(len(simultaneous))
    elif not exploratory_matches.empty:
        earliest_row = exploratory_matches.sort_values(
            "time_window_minutes"
        ).iloc[0]
        earliest_exploratory_window_minutes = float(
            earliest_row["time_window_minutes"]
        )
        earliest_exploratory_candidate_count = int(
            earliest_row["number_simultaneous_candidates"]
        )
        largest_row = exploratory_matches.sort_values(
            "time_window_minutes"
        ).iloc[-1]
        conclusion = (
            "No simultaneous candidate was found within the configured "
            f"±{args.time_margin_s / 60.0:g} minute window. The exploratory "
            f"time-window analysis first identified "
            f"{earliest_exploratory_candidate_count} plausible candidate(s) "
            f"within ±{earliest_exploratory_window_minutes:g} minutes and "
            f"identified {int(largest_row['number_simultaneous_candidates'])} "
            f"candidate(s) within ±{float(largest_row['time_window_minutes']):g} "
            "minutes. These wider-window matches are exploratory and do not "
            "prove experimental pairing."
        )
    else:
        earliest_exploratory_window_minutes = None
        earliest_exploratory_candidate_count = 0
        conclusion = (
            "No scanned measurement satisfies both temporal and spectral "
            "criteria within the configured time margin or any of the tested "
            "exploratory time windows."
        )
    ...
    summary = {
        ...
        "number_simultaneous_candidates": int(len(simultaneous)),
        "earliest_exploratory_window_minutes": earliest_exploratory_window_minutes,
        "earliest_exploratory_candidate_count": earliest_exploratory_candidate_count,
