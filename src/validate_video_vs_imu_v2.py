...
def load_video_signal(
    ...
    if function is not None:
        try:
            ...
            if config.strict_modules:
                raise
            LOGGER.warning(
                "video_loader-Funktion fehlgeschlagen; CSV-Fallback wird genutzt: %s",
                exc,
            )

    frame = _read_delimited_file(config.video_path)
    signal = _coerce_signal(
        frame,
        name=config.video_name,
        unit=config.video_unit,
    ...
    signal.metadata["loader"] = "internal_csv_fallback"
    return signal
    ...
    if function is not None:
        try:
            ...
            if config.strict_modules:
                raise
            LOGGER.warning(
                "imu_loader-Funktion fehlgeschlagen; CSV-Fallback wird genutzt: %s",
                exc,
            )

    frame = _read_delimited_file(config.imu_path)
    signal = _coerce_signal(
        frame,
        name=config.imu_name,
        unit=config.imu_unit,
...
def preprocess_signal(
    ...
    *,
    ...
    signal = _crop_signal(signal, config.start_time_s, config.end_time_s)
    ...
    if function is not None:
        try:
            ...
            if config.strict_modules:
                raise
            LOGGER.warning(
                "preprocessing.%s für %s fehlgeschlagen; Fallback: %s",
                function.__name__,
                kind,
                exc,
            )
    ...
    return _internal_preprocess(
        signal, scale=scale, offset=offset, config=config
    )
    ...
    if function is not None:
        try:
            ...
            if isinstance(result, Mapping):
                ...
                metadata = dict(result.get("metadata", result))
            elif isinstance(result, (tuple, list)) and len(result) >= 2:
                raw_video, raw_imu = result[0], result[1]
                metadata = (
                    dict(result[2])
                ...
                raise TypeError("Unbekanntes Synchronisationsergebnis.")

            aligned_video = _coerce_signal(
                raw_video,
                name=video.name,
                unit=video.unit,
                source=Path(video.source or "video"),
                time_column=None,
                value_column=None,
            ...
            return aligned_video, aligned_imu, _jsonable(metadata)
        except Exception as exc:
            if config.strict_modules:
                raise
            LOGGER.warning(
                "Synchronisationsmodul fehlgeschlagen; Fallback wird genutzt: %s",
                exc,
            )

    return _internal_synchronize(video, imu, config)
...
def compute_metrics(
    ...
    if function is not None:
        try:
            ...
            if isinstance(result, Mapping):
                metrics = dict(_jsonable(result))
            elif isinstance(result, pd.Series):
                metrics = dict(_jsonable(result.to_dict()))
            elif isinstance(result, pd.DataFrame):
                if {"metric", "value"}.issubset(result.columns):
                    metrics = dict(
                        zip(
                    ...
                    metrics = {"table": _jsonable(result)}
            elif dataclasses.is_dataclass(result):
                metrics = dict(_jsonable(dataclasses.asdict(result)))
            else:
                metrics = dict(_jsonable(vars(result)))
            metrics.setdefault(
                "implementation",
                f"{function.__module__}.{function.__name__}",
            )
            ...
            if config.strict_modules:
                raise
            LOGGER.warning(
                "metrics-Modul fehlgeschlagen; Fallback wird genutzt: %s", exc
            )

    return _internal_metrics(video, imu)
    ...
    if function is not None:
        try:
            ...
            if result is None:
                ...
                paths = [Path(path) for path in result.values()]
            ...
            if config.strict_modules:
                raise
            LOGGER.warning(
                "plotting-Modul fehlgeschlagen; Fallback wird genutzt: %s", exc
            )

    return _internal_plots(video, imu, config.output_dir)
